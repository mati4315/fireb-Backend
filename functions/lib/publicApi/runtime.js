...
const getRequestId = (req) => {
    ...
    if (/^[a-zA-Z0-9._:-]{8,80}$/.test(supplied))
        return supplied;
...
const parseLimit = (value, max = MAX_LIMIT) => {
    ...
    if (!/^\d+$/.test(raw))
        return { error: `El parámetro limit debe ser un entero entre 1 y ${max}.` };
...
const parseDate = (value, field) => {
    ...
    if (Number.isNaN(date.getTime()))
        return { error: `${field} debe ser una fecha ISO 8601 válida.` };
    return { value: admin.firestore.Timestamp.fromDate(date) };
};
const encodeCursor = (value) => Buffer.from(JSON.stringify({ created_at_ms: value.toMillis() }), 'utf8').toString('base64url');
const decodeCursor = (value) => {
    ...
    try {
        ...
        if (typeof decoded.created_at_ms !== 'number' || !Number.isFinite(decoded.created_at_ms))
            return null;
...
const parseCsv = (value) => getString(value)
    .split(',')
    .map((item) => item.trim().toLowerCase())
    .filter(Boolean);
const validateQueryKeys = (query, allowed) => {
...
const getPathParts = (req) => req.path.split('/').filter(Boolean);
...
const newsQuery = (query) => {
    ...
    if (from)
        result = result.where('createdAt', '>=', from);
    if (until)
        result = result.where('createdAt', '<=', until);
...
const matchesNewsFilters = (item, query) => {
    var _a, _b;
    const category = getString(query.category).toLowerCase();
    if (category && ((_a = item.category) === null || _a === void 0 ? void 0 : _a.id) !== category && ((_b = item.category) === null || _b === void 0 ? void 0 : _b.name.toLowerCase()) !== category)
        return false;
    return true;
};
const publicNewsList = async (req, res, requestId) => {
    const unsupportedKey = validateQueryKeys(req.query, new Set(['limit', 'cursor', 'category', 'from', 'until']));
    if (unsupportedKey)
        return errorResponse(res, 400, 'INVALID_PARAMETER', `El filtro ${unsupportedKey} no está disponible en esta versión.`, requestId, [{ field: unsupportedKey, reason: 'unsupported_filter' }]);
    const parsedLimit = parseLimit(req.query.limit);
    if (parsedLimit.error)
        return errorResponse(res, 400, 'INVALID_PARAMETER', parsedLimit.error, requestId, [{ field: 'limit', reason: 'out_of_range' }]);
...
const publicSearch = async (req, res, requestId) => {
    ...
    if (q.length < 2 || q.length > 120)
        return errorResponse(res, 400, 'INVALID_PARAMETER', 'El parámetro q debe tener entre 2 y 120 caracteres.', requestId, [{ field: 'q', reason: 'invalid_length' }]);
    const parsedLimit = parseLimit(req.query.limit, MAX_SEARCH_LIMIT);
    if (parsedLimit.error)
        return errorResponse(res, 400, 'INVALID_PARAMETER', parsedLimit.error, requestId);
    const types = parseCsv(req.query.types);
    if (types.some((type) => type !== 'news' && type !== 'event'))
        return errorResponse(res, 400, 'INVALID_PARAMETER', 'types solo admite news,event.', requestId, [{ field: 'types', reason: 'unsupported_value' }]);
    const dates = [parseDate(req.query.from, 'from'), parseDate(req.query.until, 'until')];
    const dateError = dates.find((item) => item.error);
    if (dateError === null || dateError === void 0 ? void 0 : dateError.error)
        return errorResponse(res, 400, 'INVALID_PARAMETER', dateError.error, requestId);
    ...
    for (const item of scan.items) {
        ...
        if (matches.length === 0)
            continue;
        results.push((0, mappers_1.mapPublicSearchResult)(item, matches.includes('title') ? 1 : 0.5, { kind: 'lexical', fields: matches }));
...
const checkRateLimit = (req, route) => {
